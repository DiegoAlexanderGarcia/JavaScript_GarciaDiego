...
if (opcion == 1) {
    ...
    if (seccion === 1) {
        let campo = prompt("Ingrese el campo que desea agregar:")
        let valor = prompt("Ingrese el valor para " + campo + ":")
        data.informacion_personal[campo] = valor
    } else if (seccion === 2 || seccion === 3) {
        let nuevoItem = {}
        for (let key in data[seccion][0]) {
            nuevoItem[key] = prompt("Ingrese " + key + ":")
        }
        data[seccion].push(nuevoItem)
    }
} 
